...
import { n as normalize, b as basename, d as dirname, L as LS_BLOCK_SIZE, Y as YES_REPEAT_COUNT, e as extname, N as NPM_REGISTRY_URL_SLASH, V as VERSIONS, r as resolve, a as relative, c as createHash, j as join, _ as _dynamicImport, D as DEFAULT_ENV, E as EventEmitter, M as MOCK_PID, W as Writable, R as Readable, S as ScriptEngine, f as format, g as getAllServers, h as ref, u as unref, i as addDrainListener, T as TIMEOUTS, k as closeAllServers, l as resetRefCount, m as resetActiveInterfaceCount, B as Buffer2, o as getActiveContext, p as getRefCount, q as getActiveInterfaceCount, __tla as __tla_0 } from "./index-BL39kgJt.js";
...
`);
  ...
  const touch = (args, ctx) => {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      try {
        (_b = (_a = ctx.volume).utimesSync) == null ? void 0 : _b.call(_a, p, now / 1e3, now / 1e3);
      } catch {
      }
    }
    return ok();
...
`);
  ...
  const ls = (args, ctx) => {
    ...
    const targets = positional.length > 0 ? positional.map((p) => resolvePath(p, ctx.cwd)) : [
      ctx.cwd
    ];
...
`);
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      try {
        const st = ctx.volume.statSync(p);
        if (st.isFile() && !dirOnly) fileTargets.push({
          orig: positional[i],
          path: p
        });
        else dirTargets.push({
          orig: positional[i],
          path: p
        });
      } catch {
...
`);
      ...
      } else if (args[i] === "-n" || args[i] === "-i" || args[i] === "-r" || args[i] === "-E") ;
...
`);
    ...
    const rows = lines.map((l) => typeof separator === "string" ? l.split(separator) : l.split(separator));
    const numCols = Math.max(...rows.map((r) => r.length));
    const widths = Array.from({
      length: numCols
    }, (_, ci) => Math.max(...rows.map((r) => (r[ci] ?? "").length)));
    const out = rows.map((row) => row.map((cell, ci) => cell.padEnd(ci < row.length - 1 ? widths[ci] : 0)).join(outputSep)).join("\n") + "\n";
    return ok(out);
  };
...
`);
    ...
    const out = content.split("\n").map((line) => {
      var _a;
      const prefix = all ? line : ((_a = line.match(/^(\s*)/)) == null ? void 0 : _a[1]) ?? "";
...
`;
  ...
  const od_cmd = (args, ctx, stdin) => {
    ...
    const format2 = opts["t"] || (flags.has("x") ? "x2" : flags.has("d") ? "d2" : flags.has("c") ? "c" : "o2");
...
`);
    ...
    const formatByte = (b) => {
      ...
      if (format2 === "x1" || format2 === "x" || format2 === "x2") return b.toString(16).padStart(2, "0");
      if (format2 === "d2") return String(b).padStart(3);
      if (format2 === "o2") return b.toString(8).padStart(3, "0");
...
`;
        ...
        if (failed > 0) return {
          stdout: out2,
          stderr: `WARNING: ${failed} computed checksum did NOT match
`,
          exitCode: 1
        };
        return ok(out2);
  ...
  const expr_cmd = (args) => {
    ...
    const evalMul = () => {
      ...
      while (peek() === "*" || peek() === "/" || peek() === "%") {
        ...
        if ((op === "/" || op === "%") && rn === 0) return fail("expr: division by zero\n").stderr;
...
`);
  ...
  function runAWK(program, lines, fs, filename) {
    ...
    while ((m = re.exec(program)) !== null) {
      const keyword = m[1];
      const pattern = ((_a = m[2]) == null ? void 0 : _a.trim()) || null;
      const action = m[3];
      rules.push({
  ...
  function jqEval(filter, input) {
    ...
    if (arithMatch) {
      ...
      if (op === "+") {
        ...
        if (l && typeof l === "object" && r && typeof r === "object") return [
          {
            ...l,
            ...r
          }
      ...
      if (op === "-" && typeof l === "number" && typeof r === "number") return [
        l - r
      ];
      if (op === "*" && typeof l === "number" && typeof r === "number") return [
        l * r
    ...
    if (filter === "floor") return [
      Math.floor(input)
    ];
    if (filter === "ceil") return [
      Math.ceil(input)
    ];
    if (filter === "round") return [
      Math.round(input)
    ];
    if (filter === "sqrt") return [
      Math.sqrt(input)
    ];
    if (filter === "fabs" || filter === "abs") return [
      Math.abs(input)
    ];
    if (filter === "nan") return [
      NaN
    ];
    if (filter === "infinite") return [
      Infinity
    ];
    if (filter === "isinfinite") return [
      !isFinite(input) && !isNaN(input)
    ];
    if (filter === "isnan") return [
      isNaN(input)
    ];
    if (filter === "isnormal") return [
      isFinite(input) && !isNaN(input)
    ];
    if (filter === "ascii_downcase") return [
      typeof input === "string" ? input.toLowerCase() : input
    ];
    if (filter === "ascii_upcase") return [
    ...
    if (filter === "unique") {
      ...
      return [
        [
          ...new Set(input.map((v) => JSON.stringify(v)))
        ].map((v) => JSON.parse(v))
      ];
    }
    if (filter === "flatten") {
      const flat = (a) => a.flatMap((v) => Array.isArray(v) ? flat(v) : [
        v
      ]);
      return [
        Array.isArray(input) ? flat(input) : input
      ];
    }
    if (filter === "add") {
      ...
      if (Array.isArray(input[0])) return [
        input.flat()
      ];
      return [
        input.reduce((a, b) => ({
    ...
    if (filter === "env") return [
      Object.fromEntries(Object.entries(typeof process !== "undefined" ? process.env : {}))
    ];
    if (filter === "debug") {
    ...
    if (funcMatch) {
      ...
      if (fname === "select") return jqEval(fargs, input)[0] ? [
        input
      ] : [];
      if (fname === "has") {
        const key = argVals[0];
        if (Array.isArray(input)) return [
          typeof key === "number" && key < input.length
        ];
        if (typeof input === "object" && input !== null) return [
          String(key) in input
        ];
      ...
      if (fname === "in") {
        ...
        if (typeof input === "string" && typeof obj === "object" && obj !== null) return [
          input in obj
        ];
        return [
          false
      ...
      if (fname === "contains") {
        const val = argVals[0];
        if (typeof input === "string" && typeof val === "string") return [
          input.includes(val)
        ];
        if (Array.isArray(input) && Array.isArray(val)) return [
          val.every((v) => input.includes(v))
        ];
        return [
          JSON.stringify(input).includes(JSON.stringify(val))
        ];
      }
      if (fname === "startswith") return [
        typeof input === "string" && input.startsWith(String(argVals[0]))
      ];
      if (fname === "endswith") return [
        typeof input === "string" && input.endsWith(String(argVals[0]))
      ];
      if (fname === "ltrimstr") return [
        typeof input === "string" ? input.startsWith(String(argVals[0])) ? input.slice(String(argVals[0]).length) : input : input
      ];
      if (fname === "rtrimstr") return [
        typeof input === "string" ? input.endsWith(String(argVals[0])) ? input.slice(0, -String(argVals[0]).length) : input : input
      ];
      if (fname === "split") return [
        typeof input === "string" ? input.split(String(argVals[0])) : input
      ];
      if (fname === "join") return [
        Array.isArray(input) ? input.join(String(argVals[0])) : input
      ];
      if (fname === "test") {
        try {
          return [
            new RegExp(String(argVals[0]), String(argVals[1] ?? "")).test(String(input))
          ];
        } catch {
          return [
            false
      ...
      if (fname === "scan") {
        ...
        return [
          Array.from(String(input).matchAll(re)).map((m) => m[0])
        ];
      }
      if (fname === "capture") {
        try {
      ...
      if (fname === "index") {
        if (Array.isArray(input)) return [
          input.indexOf(argVals[0])
        ];
        return [
          String(input).indexOf(String(argVals[0]))
        ];
      }
      if (fname === "rindex") {
        if (Array.isArray(input)) return [
          input.lastIndexOf(argVals[0])
        ];
        return [
          String(input).lastIndexOf(String(argVals[0]))
        ];
      }
      if (fname === "map") {
        if (!Array.isArray(input)) return [
        ...
        return [
          input.flatMap((v) => jqEval(fargs, v))
        ];
      }
      if (fname === "map_values") {
        if (Array.isArray(input)) return [
          input.map((v) => jqEval(fargs, v)[0])
        ];
      ...
      if (fname === "select") return jqEval(fargs, input).length > 0 && jqEval(fargs, input)[0] ? [
        input
      ] : [];
      if (fname === "sort_by") {
        if (!Array.isArray(input)) return [
      ...
      if (fname === "min_by") {
        ...
        return [
          input.reduce((best, v) => JSON.stringify(jqEval(fargs, v)[0]) < JSON.stringify(jqEval(fargs, best)[0]) ? v : best)
        ];
      }
      if (fname === "max_by") {
        if (!Array.isArray(input) || input.length === 0) return [
      ...
      if (fname === "first") return [
        jqEval(fargs, input)[0]
      ];
      if (fname === "last") {
      ...
      if (fname === "any") return [
        Array.isArray(input) ? input.some((v) => Boolean(jqEval(fargs, v)[0])) : Boolean(jqEval(fargs, input)[0])
      ];
      if (fname === "all") return [
        Array.isArray(input) ? input.every((v) => Boolean(jqEval(fargs, v)[0])) : Boolean(jqEval(fargs, input)[0])
      ];
      if (fname === "reduce") {
        ...
        return [
          acc.reduce((a, v) => jqEval(parts[1].trim().replace(/\baccum\b/, JSON.stringify(a)), v)[0], init)
        ];
      }
      if (fname === "flatten") {
        const depth = argVals.length > 0 ? Number(argVals[0]) : Infinity;
        const flat2 = (a, d) => d <= 0 ? a : a.flatMap((v) => Array.isArray(v) ? flat2(v, d - 1) : [
          v
        ]);
        return [
          Array.isArray(input) ? flat2(input, depth) : input
        ];
      }
      if (fname === "error") {
      ...
      if (fname === "ascii") return [
        String(input).charCodeAt(0)
      ];
      if (fname === "implode") return [
        Array.isArray(input) ? String.fromCharCode(...input) : input
      ];
      if (fname === "explode") return [
        typeof input === "string" ? [
          ...input
        ].map((c) => c.charCodeAt(0)) : input
      ];
      ...
      if (fname === "format" || fname === "@base64") return [
        btoa(String(input))
      ];
      if (fname === "gsub") {
        const re2 = new RegExp(String(argVals[0]), "g");
        return [
          typeof input === "string" ? input.replace(re2, String(argVals[1] ?? "")) : input
        ];
      }
      if (fname === "sub") {
        const re3 = new RegExp(String(argVals[0]));
        return [
          typeof input === "string" ? input.replace(re3, String(argVals[1] ?? "")) : input
        ];
      }
      if (fname === "ascii_downcase") return [
        typeof input === "string" ? input.toLowerCase() : input
      ];
      if (fname === "ascii_upcase") return [
        typeof input === "string" ? input.toUpperCase() : input
      ];
      if (fname === "ltrimstr") return [
        typeof input === "string" ? input.startsWith(String(argVals[0])) ? input.slice(String(argVals[0]).length) : input : input
    ...
    if (/^\.\w+/.test(filter)) {
      ...
      for (const part of parts) {
        ...
        if (arrMatch) {
          ...
          val = val == null ? void 0 : val[optMatch[1]];
          if (val === void 0) val = null;
        } else {
          val = val == null ? void 0 : val[part];
          if (val === void 0) val = null;
        }
      }
      return [
    ...
    if (/^\.\[-?\d+\]$/.test(filter)) {
      ...
      return [
        idx < 0 ? input[input.length + idx] : input[idx]
      ];
    }
    if (/^\.\["[^"]+"\]$/.test(filter)) {
    ...
    if (filter === "@json") return [
      JSON.stringify(input)
    ];
    if (filter === "@text" || filter === "@html") return [
      String(input)
    ];
    ...
    if (filter === "@csv") {
      ...
      return [
        input.map((v) => typeof v === "string" ? `"${v.replace(/"/g, '""')}"` : String(v)).join(",")
      ];
    }
    if (filter === "@tsv") {
      if (!Array.isArray(input)) return [
      ...
      return [
        input.map((v) => String(v).replace(/\t/g, "\\t")).join("	")
      ];
    }
    if (filter === "@sh") return [
      typeof input === "string" ? `'${input.replace(/'/g, "'\\''")}'` : JSON.stringify(input)
    ];
    if (filter.startsWith('"') && filter.endsWith('"')) {
...
`);
    ...
    const envToPrint = clearEnv ? envOverrides : {
      ...ctx.env,
      ...envOverrides
    };
    let out = "";
...
`;
  ...
  const df_cmd = (args) => {
    ...
    return ok(header + `wenode         ${typeCol}${fmt(total)} ${fmt(used)} ${fmt(avail)}  ${pct}% /
`);
  };
...
`);
  ...
  const kill_cmd = (args) => {
    if (args.includes("-l") || args.includes("--list")) {
      return ok(" 1 HUP  2 INT  3 QUIT  6 ABRT  9 KILL 15 TERM 17 CHLD 18 CONT 19 STOP\n");
    }
    return ok();
  };
  const pkill_cmd = () => ok();
  ...
  const ulimit_cmd = (args) => {
    ...
    if (flags.has("a")) {
      return ok("core file size          (blocks, -c) 0\ndata seg size           (kbytes, -d) unlimited\nfile size               (blocks, -f) unlimited\nopen files                      (-n) 1024\nstack size              (kbytes, -s) 8192\ncpu time               (seconds, -t) unlimited\nvirtual memory          (kbytes, -v) unlimited\n");
    }
  ...
  const mktemp_cmd = (args, ctx) => {
    ...
    const name = template.replace(/X{3,}$/, () => {
      var _a;
      return rand().slice(0, ((_a = template.match(/X+$/)) == null ? void 0 : _a[0].length) ?? 6);
    });
    const base = ctx.env.TMPDIR || "/tmp";
    const p = `${base}/${name}`;
    try {
      ctx.volume.mkdirSync(base, {
        recursive: true
      });
      if (isDir) {
        ctx.volume.mkdirSync(p, {
...
`);
  ...
  const time_cmd = async (args, ctx) => {
    ...
    const result = await ctx.exec(args.join(" "), {
      cwd: ctx.cwd,
      env: ctx.env
    });
...
`,
  ...
  const timeout_cmd = async (args, ctx) => {
    ...
    const result = await Promise.race([
      ...
      new Promise((resolve2) => setTimeout(() => resolve2({
        stdout: "",
        stderr: "",
        exitCode: 124
      }), seconds * 1e3))
...
`);
  ...
  class WeNodeShell {
    constructor(volume, opts) {
      ...
      __publicField(this, "_heredocStack", []);
      this.volume = volume;
    ...
    async exec(command, opts) {
      (opts == null ? void 0 : opts.cwd) ?? this.cwd;
        ...
        for (const r of cmd.redirects) {
          if (r.type === "read") {
            if (r.target.charCodeAt(0) === 1) {
              ...
              const hd = heredocMap == null ? void 0 : heredocMap.get(r.target);
...
`,
    ...
    normalizeCommandName(name) {
      ...
      const virtualBinDirs = /* @__PURE__ */ new Set([
        "/bin",
        "/usr/bin",
        "/usr/local/bin"
      ]);
...
`);
  ...
  async function loadPyodide() {
    ...
    pyodideLoadPromise = (async () => {
      try {
        const pyodideModule = await _dynamicImport("https://cdn.jsdelivr.net/pyodide/v0.25.1/full/pyodide.mjs");
        ...
        if (volume.existsSync(volumePath)) {
          ...
          if (stats.isFile()) {
            ...
            for (const entry of entries) {
              const volEntryPath = join(volumePath, entry);
              const pyEntryPath = join(pyodidePath, entry);
...
`;
            ...
            const resolvedPath = target.startsWith("/") ? target : join(cwd, target);
...
`;
          ...
          return {
            stdout: "",
            stderr: `python: ${(e == null ? void 0 : e.message) || String(e)}
`,
            exitCode: 1
          };
        }
...
`;
  ...
  async function installPackages(args, ctx, pm = "npm") {
    const { DependencyInstaller } = await import("./index-BL39kgJt.js").then(async (m) => {
      await m.__tla;
      return m;
    }).then((n) => n.ag);
    ...
    try {
      ...
      const persistentDebugLines = out.split("\n").filter((line) => line.startsWith("Install stamp check:") || line.startsWith("Install stamp hydrate:") || line.startsWith("Wrote OPFS install stamp") || line.startsWith("OPFS stamp skipped:") || line.startsWith("Reused OPFS install stamp (fast path)"));
...
`;
    ...
    try {
      const { RegistryClient } = await import("./index-BL39kgJt.js").then(async (m) => {
        await m.__tla;
        return m;
      }).then((n) => n.av);
...
`,
  ...
  execSync = function(cmd, opts) {
    ...
    if (!_syncChannel) {
      throw new Error("[WeNode] execSync requires SyncChannel (worker mode with SharedArrayBuffer). Ensure WeNode is running in worker mode with COOP/COEP headers.");
    }
...
`);
  ...
  function handleSyncCommand(cmd, opts) {
    ...
    if (/^uname\s+-a\s*$/.test(cmd)) return "Linux weNode 5.10.0 #1 SMP x86_64 GNU/Linux\n";
    ...
    if (_spawnChildFn) {
      ...
      _spawnChildFn(command, spawnArgs, {
        ...
        if (!streamedStdout && stdout) {
          (_a = child.stdout) == null ? void 0 : _a.push(Buffer2.from(stdout));
          const sink = getStdoutSink();
          if (sink) sink(stdout);
        }
        if (!streamedStderr && stderr) {
          (_b = child.stderr) == null ? void 0 : _b.push(Buffer2.from(stderr));
          const sink = getStderrSink();
          if (sink) sink(stderr);
        }
        (_c = child.stdout) == null ? void 0 : _c.push(null);
  ...
  spawnSync = function(cmd, args, opts) {
    ...
    if (!_syncChannel) {
      throw new Error("[WeNode] spawnSync requires SyncChannel (worker mode with SharedArrayBuffer). Ensure WeNode is running in worker mode with COOP/COEP headers.");
    }
    const slot = _syncChannel.allocateSlot();
