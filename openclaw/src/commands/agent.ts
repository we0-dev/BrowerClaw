...
function toPrintableOutput(payload: unknown): string {
  ...
  if (typeof record.mediaUrl === "string" && record.mediaUrl.trim()) {
    lines.push(`MEDIA:${record.mediaUrl.trim()}`);
  }
...
export async function agentCommand(
  ...
  _deps: CliDeps = createDefaultDeps(),
) {
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(`Unknown agent id "${agentIdOverrideRaw}". Check your config file.`);
    }
  }
  ...
  const workspaceDirRaw = opts.workspace?.trim()
    ? opts.workspace.trim()
  ...
  try {
    ...
    const skillsSnapshot = needsSkillsSnapshot
      ? buildWorkspaceSkillSnapshot(workspaceDir, {
          config: cfg,
          eligibility: getRemoteSkillEligibility(),
    ...
    try {
      ...
      const messageChannel = runContext.messageChannel;
    ...
    for (const payload of payloads) {
      const output = toPrintableOutput(payload);
