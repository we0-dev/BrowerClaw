...
export type AgentCommandOpts = {
  ...
  /** Optional runtime source label for the current agent turn. */
  messageChannel?: string;
  channel?: string;
  ...
  /**
   * Override the agent workspace directory for this run.
   * Useful for virtualized environments where each "chat/task" has its own workspace.
   */
  workspace?: string;
};
