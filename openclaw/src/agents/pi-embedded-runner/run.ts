...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      const applyApiKeyInfo = async (): Promise<void> => {
        apiKeyInfo = await getApiKeyForModel({
          model,
          cfg: params.config,
          agentDir,
        });
        const resolvedProfileId = apiKeyInfo.profileId;
      ...
      try {
        ...
        const reason = classifyFailoverReason(message) ?? "auth";
      ...
      try {
        while (true) {
          ...
          const shouldFailoverModel = (!aborted && failoverFailure) || timedOut;
          if (shouldFailoverModel) {
            if (timedOut && !isProbeSession) {
              log.warn(`Model ${provider}/${modelId} timed out.`);
            }
            if (cloudCodeAssistFormatError) {
              log.warn(
                `Model ${provider}/${modelId} hit Cloud Code Assist format error. Tool calls will be sanitized on retry.`,
              );
            }
            if (fallbackConfigured) {
              const message =
