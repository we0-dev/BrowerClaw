...
export async function resolveApiKeyForProvider(params: {
  ...
  if (params.profileId) {
    throw new Error(
      `Auth profiles are not supported in the mini build (requested: "${params.profileId}").`,
    );
  }
  ...
  throw new Error(
    [
      ...
      `Checked env vars and models.providers.${provider}.apiKey.`,
      `Agent dir: ${params.agentDir ? path.resolve(params.agentDir) : "(default)"}.`,
    ].join(" "),
  );
}
