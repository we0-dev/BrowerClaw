...
import { getCustomProviderApiKey, resolveAwsSdkEnvVarName, resolveEnvApiKey } from "./model-auth.js";
...
export async function resolveImplicitProviders(params: {
  ...
  if (resolveEnvApiKeyVarName("qwen-portal")) {
...
export async function resolveImplicitCopilotProvider(params: {
  ...
  // Copilot auth is still exchanged at runtime, so we also write a runtime-only
  // auth.json entry for pi-coding-agent to pick up.
  //
  // This is safe because it's (1) within OpenClaw's agent dir, (2) contains the
  // GitHub token (not the exchanged Copilot token), and (3) matches existing
  // patterns for OAuth-like providers in pi-coding-agent.
  // Note: we deliberately do not write pi-coding-agent's `auth.json` here.
  // OpenClaw exchanges tokens at runtime.
