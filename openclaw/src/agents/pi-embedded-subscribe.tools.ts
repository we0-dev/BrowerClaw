...
export function extractMessagingToolSend(
  ...
  const providerRaw = typeof args.provider === "string" ? args.provider.trim().toLowerCase() : "";
  const accountIdRaw = typeof args.accountId === "string" ? args.accountId.trim() : undefined;
  const to = normalizeTargetForProvider(providerRaw || "message", toRaw);
  return to
    ? {
        tool: toolName,
        provider: providerRaw || "message",
