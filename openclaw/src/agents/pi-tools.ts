...
export function createOpenClawCodingTools(options?: {
  ...
  const toolsFiltered = profilePolicyWithAlsoAllow
    ? filterToolsByPolicy(tools, profilePolicyWithAlsoAllow)
    : tools;
  const providerProfileFiltered = providerProfilePolicyWithAlsoAllow
    ? filterToolsByPolicy(toolsFiltered, providerProfilePolicyWithAlsoAllow)
    : toolsFiltered;
  ...
  const agentProviderFiltered = agentProviderPolicy
    ...
    : agentFiltered;
  const groupFiltered = groupPolicy
    ? filterToolsByPolicy(agentProviderFiltered, groupPolicy)
    : agentProviderFiltered;
  const sandboxed = sandbox?.tools
    ? filterToolsByPolicy(groupFiltered, sandbox.tools)
    : groupFiltered;
